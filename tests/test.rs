...
fn write_blob() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::is_match(SHA_REGEX)?);
...
fn clone_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let project_path = Path::new("C:\\Users\\iyashan\\rust\\owngit\\08bd1dc1a640270a\\tests_temp");
    ...
    println!("temp_folder_2: {:?}", temp_folder_2);
    ...
    let temp_folder_1 = project_path.join("MyGit");
    println!("temp_folder_1: {:?}", temp_folder_1);
    ...
    println!("Validating working tree contents");
    let excluded = vec![".git".to_string()];

    let cmp_result = FolderCompare::new(
        &temp_folder_2, // Compare from
        &temp_folder_1, // Compare to
        &excluded,
    )
    .unwrap();

    println!("cmp_result.changed_files: {:#?}", cmp_result.changed_files);
    //println!("cmd.output: {}", String::from_utf8(cmd.output()?.stdout)?);
    ...
    let cmp_result = FolderCompare::new(
        &temp_folder_1.join(".git/refs/heads"),
        &temp_folder_2.join(".git/refs/heads"),
        &vec![],
    )
    .unwrap();
    ...
    assert_eq!(
        fs::read(temp_folder_1.join(".git/HEAD"))?,
        fs::read(temp_folder_2.join(".git/HEAD"))?
    );
